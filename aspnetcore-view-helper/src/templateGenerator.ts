...
export async function generateViewFile(options: ViewGenerationOptions): Promise<void> {
	...
	let modelProperties: import('./types').ModelProperty[] = [];
	if (modelType) {
		modelProperties = await parseModelProperties(modelType, projectRoot);
	}
	...
	const templateData: TemplateData = {
		...
		primaryKeyProperty: modelProperties.find(prop => prop.isPrimaryKey)?.name
	};
